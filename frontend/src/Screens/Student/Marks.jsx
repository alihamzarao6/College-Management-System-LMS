    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data.success) {
          const marks = await response.data.Mark;
          if (marks && marks.length > 0) {
            setInternal(marks[0].internal);
            setExternal(marks[0].external);
          } else {
            toast.success("No marks available for the specified enrollment number.");
          }
        } else {
          toast.error("Failed to load marks. Please try again later.");
        }
      } catch (error) {
        toast.error("An error occurred while loading marks. Please try again later.");
        console.error(error);
      }
    };
